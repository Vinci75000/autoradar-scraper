"""scripts/run_classictrader_test.py — End-to-end pipeline test for ClassicTrader.

Runs the full chain:
  ClassicTraderExtractor.extract(config, limit=N)
    -> for each CarListing:
       -> insert_car(db, car)
    -> aggregate outcomes (inserted, duplicate, rejected, errors)

Usage:
    python -u scripts/run_classictrader_test.py --limit 10
    python -u scripts/run_classictrader_test.py --limit 10 --dry-run  # extract only, no DB writes
"""
...
def main() -> int:
    ...
    if inserted_ids:
        ...
        print(f"   FROM cars WHERE src='classictrader' ORDER BY created_at DESC LIMIT 10;")
