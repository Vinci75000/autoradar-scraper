"""scripts/run_classicdriver_test.py — End-to-end pipeline test for ClassicDriver.

Runs the full chain:
  ClassicDriverExtractor.extract(config, limit=N)
    -> for each CarListing:
       -> insert_car(db, car)
    -> aggregate outcomes (inserted, duplicate, rejected, errors)

Usage:
    python -u scripts/run_classicdriver_test.py --limit 10
    python -u scripts/run_classicdriver_test.py --limit 10 --dry-run  # extract only, no DB writes
"""
...
from extractors.classicdriver import ClassicDriverExtractor
...
def main() -> int:
    ...
    config = SourceConfig(
        slug="classicdriver",
        listings_url="https://classicdriver.com/sitemap.xml",
        country="gb",
        currency="EUR",
        language="en",
        timezone="UTC",
    ...
    print(f">> running classicdriver pipeline test - limit={args.limit} dry_run={args.dry_run}\n",
          flush=True)
    ...
    if inserted_ids:
        ...
        print(f"   FROM cars WHERE src='classicdriver' ORDER BY created_at DESC LIMIT 10;")
