"""scripts/run_carandclassic_test.py — End-to-end pipeline test for CarAndClassic.

Runs the full chain:
  CarAndClassicExtractor.extract(config, limit=N)
    -> for each CarListing:
       -> insert_car(db, car)
    -> aggregate outcomes (inserted, duplicate, rejected, errors)

Usage:
    python -u scripts/run_carandclassic_test.py --limit 10
    python -u scripts/run_carandclassic_test.py --limit 10 --dry-run  # extract only, no DB writes
"""
...
def main() -> int:
    ...
    config = SourceConfig(
        slug="carandclassic",
        listings_url="https://carandclassic.com/sitemap.xml",
        country="gb",
        currency="EUR",
        language="en",
        timezone="UTC",
    ...
    print(f">> running carandclassic pipeline test - limit={args.limit} dry_run={args.dry_run}\n",
          flush=True)
    ...
    if inserted_ids:
        ...
        print(f"   FROM cars WHERE src='carandclassic' ORDER BY created_at DESC LIMIT 10;")
