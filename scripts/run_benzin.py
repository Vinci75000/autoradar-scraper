"""scripts/run_benzin.py — End-to-end pipeline runner for Benzin.fr.

Runs the full chain:
  BenzinExtractor.extract(config, limit=N)
    -> for each CarListing:
       -> insert_car(db, car)
    -> aggregate outcomes (inserted, duplicate, rejected, errors)

Usage:
    python -u scripts/run_benzin.py --limit 10
    python -u scripts/run_benzin.py --limit 10 --dry-run
"""
...
from extractors.benzin import BenzinExtractor


def parse_args():
    p = argparse.ArgumentParser()
    p.add_argument("--limit", type=int, default=None,
                   help="Cap on number of auction lots (default: None = full sitemap)")
...
def main() -> int:
    ...
    print(f">> running benzin pipeline - limit={args.limit} dry_run={args.dry_run}\n",
          flush=True)

    extractor = BenzinExtractor()
    ...
    print(f"\n>> INSERTION DONE in {insert_duration:.2f}s")
