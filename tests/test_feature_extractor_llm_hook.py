"""Tests pour le hook LLM dans extract_features (Phase 4).

9 tests d'integration couvrant les conditions d'activation et le
silent fallback. Tous mockes via unittest.mock.patch sur
extractors.llm_extractor.extract_features_via_llm — aucun appel
reseau reel en CI.
...
def test_llm_hook_disabled_no_call(monkeypatch):
    ...
    with patch('extractors.llm_extractor.extract_features_via_llm') as mock_llm:
        ...
        result = extract_features(description=_long_description(), title="Test")
...
def test_llm_hook_disabled_explicit_false(monkeypatch):
    ...
    with patch('extractors.llm_extractor.extract_features_via_llm') as mock_llm:
        from extractors.feature_extractor import extract_features
        extract_features(description=_long_description(), title="Test")
...
def test_llm_hook_no_description_no_call(monkeypatch):
    ...
    with patch('extractors.llm_extractor.extract_features_via_llm') as mock_llm:
        from extractors.feature_extractor import extract_features
        result = extract_features(description="", title="Test title")
...
def test_llm_hook_description_too_short_no_call(monkeypatch):
    """Flag ON mais description < LLM_HOOK_DE_LEN_MIN (800) -> pas d'appel."""
    monkeypatch.setenv('AUTORADAR_LLM_HOOK_ENABLED', 'true')

    with patch('extractors.llm_extractor.extract_features_via_llm') as mock_llm:
        from extractors.feature_extractor import extract_features
        extract_features(description=_short_description(), title="Test")
...
def test_llm_hook_v1_signal_detected_no_call(monkeypatch):
    ...
    # Donc le hook LLM n'a pas ete declenche
    mock_llm.assert_not_called()
