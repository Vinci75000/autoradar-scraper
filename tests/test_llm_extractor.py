...
def _valid_response_text() -> str:
    """Retourne un texte de reponse JSON valide avec tous les features."""
    ...
    if 'feat_first_owner' in features:
        features['feat_first_owner'] = True
...
# ===========================================================================
# Tests helpers prives -- _parse_response
# ===========================================================================
...
def test_parse_response_strips_markdown_fences_with_json_lang():
    ...
    text = (
        '```json\n'
        '{"features": {}, "highlights": [], "concerns": [], "summary": "ok"}\n'
        '```'
    )
...
def test_parse_response_missing_keys_raises():
    ...
    from extractors.llm_extractor import _parse_response
    text = '{"features": {}, "summary": "ok"}'  # missing highlights, concerns
    with pytest.raises(ValueError, match='missing keys'):
        _parse_response(text)


def test_parse_response_wrong_type_raises():
    """Mauvais type pour 'features' (str au lieu de dict) -> ValueError 'must be dict'."""
    from extractors.llm_extractor import _parse_response
    text = (
        '{"features": "not a dict", "highlights": [], '
        '"concerns": [], "summary": "ok"}'
    )
...
def test_extract_features_via_llm_success(mock_anthropic_cls):
    ...
    assert result['features']['feat_carnet_complet'] is True
    assert result['features']['feat_first_owner'] is True
